...
Total_Cor[ , colnames(Total_Cor) != 'altUnique' ]
...
DT %>% mutate_if(is.factor, as.character) -> DT
row.names(DT) <- paste0( DT[,1], ':', DT[,2] )

Total_Cor$JS <- DT[ Total_Cor$Edge, ] 

#Make iGraph net and cluster
JS_net_undirected <- graph_from_data_frame(d=Total_Cor, vertices=Genes, directed=F) 
# Louvain Clustering
Start <- Sys.time()
JS_Clust_bt <- cluster_louvain(JS_net_undirected, weights = E(net_undirected)$JS)
Sys.time()-Start
...
#####################################
#Look for enrichment in JS cluster object
Vect <- rep(NA, length(JS_Clust_bt))
for( i in 1:length(JS_Clust_bt) ){
  PVal <- pbinom(length( big[ big %in% JS_Clust_bt[[i]] ] ), 
       size=length(JS_Clust_bt[[i]] ), 
       prob=length(big)/length( V(JS_net_undirected)),
       lower.tail = F
  )
  Vect[i] <- PVal
}
...
message( paste0('Minimum P-Val of Geneset Enrichment in a module: ',  min(Vect) ))
N <- which( Vect == min(Vect) )
JS_Clust_bt[[N]]
...
message( paste0('Minimum P-Val of Geneset Enrichment in a module: ',  min(Vect) ))
#N <- which( Vect == min(Vect[ Vect != min(Vect)]) )
N <- which( Vect == min(Vect) )

JS_Clust_bt[[N]]

table( poo %in% JS_Clust_bt[[N]] )
table( big %in% JS_Clust_bt[[N]] )

##############################################################################
#Non-JS
vect <- rep(NA, length(Clust_bt))
for( i in 1:length(Clust_bt) ){
  PVal <- pbinom( length( big[ big %in% Clust_bt[[i]] ] ), 
       size=length(Clust_bt[[i]] ), 
       prob=length(big)/length( V(net_undirected)),
       lower.tail = F
  )
  vect[i] <- PVal
}
...
#which( vect_cor < 0.05 )
message( paste0('Minimum P-Val of Geneset Enrichment in a module: ',  min(vect) ))
nn <- which(  vect == min(vect) )
Clust_bt[[nn]]

table( poo %in% Clust_bt[[nn]] )
table( big %in% Clust_bt[[nn]] )
##Kinases not in graph: "TSSK3", "SNX16", "NEK11", "MINK1", "PKMYT1", "TESK2", "MELK", "DYRK4", "GK2", "MAGI2", "MAST2", "TESK1", "AURKB", "MAPKAPK5"
...
ModSiz <- NULL
for( i in 1:length(JS_Clust_bt) ){
  ModSiz <- c(ModSiz, length(Clust_bt[[i]]))
