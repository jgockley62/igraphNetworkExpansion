...
MiniNet <- Trial.path #induced.subgraph( Trial.path, as_ids(news.path$vpath[[1]]) )
MiniNet <- MiniNet %>%
  set_edge_attr("weight",  value = as.numeric(c(E(Trial.path)$Occurance))/11 ) %>%
  set_edge_attr( "color", value = c( Colors[ c(E(Trial.path)$interaction) ]) ) 
  
par(xpd=TRUE)
plot( MiniNet, edge.width = E(g)$weight, edge.arrow.size=.4 )
legend('topleft', legend=names(Colors), col= "black", pt.cex = 2, pch=21, pt.bg=Colors, bty='n')

#Set variable size of edges based on pathway redundancy
