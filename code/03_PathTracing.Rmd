...
for (i in 1:length(leading_edge)){
  ...
  allgo_genes[[leading_edge[[i]][2]]] <- g_list
  allgo_subnest[[leading_edge[[i]][2]]]  <- induced_subgraph( network, v=V(network)[ names(V(network)) %in% subset ] )
...
for (i in 1:length(all_goterms)){
  ...
  #store the genelists and subnets
  allgo_genes[[all_goterms[[i]][2]]] <- g_list
  allgo_subnest[[all_goterms[[i]][2]]]  <- induced_subgraph( network, v=V(network)[ names(V(network)) %in% subset ] )
