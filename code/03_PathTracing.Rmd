...
for (i in 1:length(all_goterms)){
  ...
  allgo_subnets[[all_goterms[[i]][2]]] <- induced_subgraph(
    network,
    v=V(network)[names(V(network)) %in% g_list]
  )
...
for (i in 1:length(leading_edge)){
  ...
  #store the genelists and subnets
  leadingedge_genes[[leading_edge[[i]][2]]] <- g_list
  leadingedge_subnets[[leading_edge[[i]][2]]] <- induced_subgraph(
    network,
    v=V(network)[names(V(network)) %in% g_list]
  )
