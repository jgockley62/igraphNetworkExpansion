...
short_paths <- function( tnet, target, targets, sentinals ){
  ...
  if ((length(sent_paths$res) == 1) & (length(sent_paths$res[[1]]) == 1)) {
    sent_keep_verticies <- NA
    ...
    if ((length(paths$res) == 1) & (length(paths$res[[1]]) == 1)) {
      keep_verticies <- NA
    }else{
      keep_verticies <- c(0,0,0)
      ...
      target_summary <- summary(
        sent_scores[(scores > 0) & (is.na(scores) == F)]
      )
  ...
  if ((length(keep_verticies) == 1) & 
      is.na(keep_verticies)[1]) {
  ...
  ## Sentinel path filtering
  if ((length(sent_keep_verticies) == 1) & 
      is.na(sent_keep_verticies)[1]) {
         message('Sentinel Path Trace Returned No Paths')
  }else{
    # Filter the Sentinel Paths
    ...
    sent_keep_verticies <- sent_keep_verticies[!(is.na(sent_keep_verticies))]
...
this_repo <- githubr::getRepo(repository = repo, ref="branch", refName='master')
prov <- githubr::getPermlink(repository = this_repo, repositoryPath = code)
