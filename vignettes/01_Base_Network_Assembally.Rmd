...
for(type in levels(total$interaction) ){
    eval(parse(text=paste0(
      'graph$`',
      type,
      '` <- graph_from_data_frame(d=Total[ Total$interaction == \'',
      type,
      '\', ], vertices=Genes, directed=T) '
    )))
}

#> Finalize the dataframe object and base network
network <- graph_from_data_frame(d=total, vertices=genes, directed=T) 
data <- list(
  data_frame=total,
  network = network,
  graph = graph
