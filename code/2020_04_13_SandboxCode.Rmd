...
#Subgraph:
Trial.path <- induced_subgraph( net_oldStyle, c( "SMOC1", "MSN", "TREM2",  "APP" ) )
...
for( name in names(Total_Occurence)){
  Total_Occurence[name] <- SumTally( name,Total )
}

Total$SumOccurence <- Total_Occurence[ Total$Edge ]

cl <- makeCluster( detectCores()-2 )
registerDoParallel(cl)
TOs <- foreach( i=names(Total_Occurence)[1:5], .combine=rbind ) %dopar% SumTally( i,Total )
stopCluster(cl)
...
net_undirected <- as.undirected(net_oldStyle)
simpnet_undirected <- igraph::simplify(net_undirected)
...
Clust_fg <- cluster_fast_greedy(slim, merges = TRUE, modularity = TRUE, membership = TRUE, weights = NULL)

message( paste0( "Found ", length(Clust_fg), " Clusters" ) )
...
# EIGEN CLUSTERING
Clust_le <- cluster_leading_eigen(slim, steps = -1, weights = , start = NULL, options = arpack_defaults, callback = NULL, extra = NULL, env = parent.frame())
